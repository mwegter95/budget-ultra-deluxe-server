...
class CategoryService extends Service {
  async getList(query, offset, limit) {
    ...
    const res = await ctx.model.Category.findAll(queryOption);
    result.data = res;

    if (hasPaging) {
      // 获取总数
      const total = await ctx.model.Category.findAndCountAll({
        where
      });
      if (total) {
